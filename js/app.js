...
.on(“postgres_changes”, { event: “*”, schema: “public”, table: “bokser” }, async () => {
await lastInnBokser();
})
.subscribe();
}
...
return `<tr class="item-row row-${sk(g.status)}${sel ? " row-selected" : ""}" data-id="${g.id}"> <td class="tc"><input type="checkbox" class="row-checkbox" value="${g.id}" onchange="oppdaterValg()" ${sel ? "checked" : ""}></td> <td class="tc"><a class="item-link mono" href="item.html?id=${enc(g.id)}">${g.id}</a></td> <td class="tc"><span class="item-name-link" style="cursor:pointer" onclick="apneRediger('${g.id}')">${g.navn}</span></td> <td class="tc"> <select class="inline-status status-select-${sk(g.status)}" onchange="raskStatus('${g.id}',this)"> ${STATUSER.map(s => `<option${s===g.status?” selected”:””}>${s}</option>`).join("")} </select> </td> <td class="tc"> <div class="actions-cell"> <button class="btn-action btn-edit"   onclick="apneRediger('${g.id}')">✏️</button> <button class="btn-action btn-qr" onclick="event.shiftKey ? toggleKo('${g.id}') : apneQr('${g.id}','${sn}')" title="Shift+klikk for å legge til/fjerne fra utskriftskø">⬛</button> <button class="btn-action btn-delete" onclick="event.shiftKey ? bekreftSlett('${g.id}','${sn}',true) : bekreftSlett('${g.id}','${sn}')">🗑️</button> </div> </td> <td class="tc col-extra col-extra-text mono small-text">${g.hylleplassering || '<span class="dash">—</span>'}</td> <td class="tc col-extra col-extra-text">${g.kategori || "—"}</td> <td class="tc col-extra col-extra-text small-text">${g.utlant_til || '<span class="dash">—</span>'}</td> <td class="tc col-extra col-extra-text mono small-text">${g.serienummer || "—"}</td> </tr>`;
}).join(””);
}
...
const modal = document.getElementById(“batchRedigerModal”);
// Populate selects
const katSel = document.getElementById(“batchKategori”);
katSel.innerHTML = ‘<option value="">— Ikke endre —</option>’ +
KATEGORIER.map(k => `<option value="${k}">${k}</option>`).join(””);
const boksSel = document.getElementById(“batchBoksId”);
boksSel.innerHTML = ‘<option value="">— Ikke endre —</option><option value="__fjern__">Fjern fra boks</option>’ +
bokser.map(b => `<option value="${b.id}">${b.navn} (${b.id})</option>`).join(””);
...
const st = document.getElementById(“batchBildeStatus”);
...
const hylle   = document.getElementById(“batchHylle”).value.trim();
const kat     = document.getElementById(“batchKategori”).value;
const boksVal = document.getElementById(“batchBoksId”).value;
const status  = document.getElementById(“batchStatus”).value;
const bildeRaw = document.getElementById(“batchBildeUrl”).value.trim();
const bilde   = bildeRaw.startsWith(“data:”) ? bildeRaw : konverterBildeUrl(bildeRaw);

const btn = document.querySelector(”#batchRedigerModal .btn-save”);
...
await html5QrScanner.start(
  ...
  (decodedText) => { behandleQrResultat(decodedText); },
  () => {}
);

// After camera starts, apply continuous focus via track constraints
setTimeout(async () => {
  try {
    const video = document.querySelector("#html5qr-scanner video");
    if (video && video.srcObject) {
      ...
      if (caps.zoom) {
        constraints.zoom = caps.zoom.min || 1;
      }
      await track.applyConstraints(constraints);
    }
  } catch(_) {}

  // Periodically nudge focus: briefly switch to manual then back to continuous
  // This forces the camera to re-evaluate and re-focus
  fokusTriggerInterval = setInterval(async () => {
    try {
      const video = document.querySelector("#html5qr-scanner video");
...
gain.gain.setValueAtTime(0.18, ctx.currentTime + 0.08);
...
clearInterval(fokusTriggerInterval);
fokusTriggerInterval = null;
if (html5QrScanner) {
try {
...
// Marker eksisterende utlån som levert hvis status endres fra Utlånt
if (gammelStatus === “Utlånt” && ny !== “Utlånt”) {
await db.from(“utlanslogg”)
.update({ status: “levert”, levert_dato: new Date().toISOString().split(“T”)[0] })
.eq(“gjenstand_id”, id).eq(“status”, “aktiv”);
}
Object.assign(g, ok); oppdaterStats(); viseSammendrag(); visBanner(“✓ “ + ny, “success”);
}
}
...
document.getElementById(“editErBulk”).checked          = !!g.er_bulk;
document.getElementById(“editAntallTotalt”).value      = g.antall_totalt || 1;
document.getElementById(“editAntallUtlant”).value      = g.antall_utlant || 0;
document.getElementById(“bulkFields”).style.display   = g.er_bulk ? “block” : “none”;
populerBoksSelect();
...
const erBulk = document.getElementById(“editErBulk”).checked;
const oppdatert = {
id: editingId,
...
er_bulk:          erBulk,
antall_totalt:    erBulk ? parseInt(document.getElementById(“editAntallTotalt”).value) || 1 : 1,
antall_utlant:    erBulk ? parseInt(document.getElementById(“editAntallUtlant”).value) || 0 : 0,
status, utlant_til, utlansdato, innleveringsdato: innlevering,
notater: document.getElementById(“editNotater”).value.trim(),
};
const gammel = gjenstander.find(x => x.id === editingId);
const { error } = await db.from(“gjenstander”).upsert(oppdatert, { onConflict: “id” });
if (error) { visBanner(“Feil: “ + error.message, “error”); }
else {
// Skriv til utlanslogg hvis status ble satt til Utlånt og det ikke allerede er et aktivt utlån
const varUtlant = gammel?.status === “Utlånt”;
if (status === “Utlånt” && utlant_til && !varUtlant) {
await db.from(“utlanslogg”).insert({
gjenstand_id: editingId,
boks_id: oppdatert.boks_id || null,
antall: 1,
utlant_til,
utlansdato,
innleveringsdato: innlevering || null,
status: “aktiv”
});
}
// Marker eksisterende utlånslogg som levert hvis status endret fra Utlånt til noe annet
if (varUtlant && status !== “Utlånt”) {
await db.from(“utlanslogg”)
.update({ status: “levert”, levert_dato: new Date().toISOString().split(“T”)[0] })
.eq(“gjenstand_id”, editingId)
.eq(“status”, “aktiv”);
}
await lastInn();
lukkRedigerModal();
...
const boksId  = document.getElementById(“itemBoksId”)?.value || null;
const erBulk  = document.getElementById(“itemErBulk”)?.checked || false;
const antTot  = erBulk ? (parseInt(document.getElementById(“itemAntallTotalt”)?.value) || 1) : 1;
...
document.getElementById(“addBulkFields”).style.display = “none”;
oppdaterPrev();
await lastInn();
lukkLeggTilModal();
}
...
// Split into pages of 24 (3 cols × 8 rows)
const PER_SIDE = 24;
const sider = [];
...
document.getElementById(“printArea”).innerHTML = sider
.map((side, idx) => {
const last = idx === sider.length - 1;
return `<div class="print-side${last ? "" : " print-side-break"}">${side.join("")}</div>`;
})
.join(””);
...
return `<div class="print-label"> <div class="print-label-qr">${qrHTML}</div> <div class="print-label-info"> <div class="print-label-navn">${g.navn}</div> <div class="print-label-id">${g.id}</div> ${g.hylleplassering ? `<div class="print-label-hylle">📍 ${g.hylleplassering}</div>` : “”}
...
function leggIKo() {
if (!_qrAktivId) return;
if (_qrAktivId.startsWith(“BOKS:”)) { visBanner(“Bokser legges ikke i utskriftskø herfra”, “error”); return; }
...
const g = gjenstander.find(x => x.id === id);
if (!g) return “”;
return `<div class="ko-chip"> <span class="ko-chip-id">${g.id}</span> <span>${g.navn}</span> <button class="ko-chip-fjern" onclick="fjernFraKo('${id}')" title="Fjern">×</button> </div>`;
}).join(””);
}
...
kb.innerHTML += `<tr> <td class="kat-navn" style="padding-left:16px">${kat}</td> <td class="center">${items.length}</td> <td class="center green-text fw">${kT}</td> <td class="center">${kU > 0 ? `<span class="red-text fw">${kU}</span>`: "0"}</td> <td class="center">${kR > 0 ?`<span class="orange-text">${kR}</span>` : "0"}</td> <td class="center muted-text">${kL}</td> <td style="padding-right:16px"><div class="andel-row"><div class="andel-bar"><div class="andel-fill" style="width:${pst}%;background:${farge}"></div></div><span class="andel-pst">${pst}%</span></div></td> </tr>`;
});
...
return `<tr class="${f ? "row-forfalt" : ""}"> <td class="mono" style="padding-left:16px"><a class="item-link" href="item.html?id=${enc(g.id)}">${g.id}</a></td> <td>${g.navn}</td><td>${g.utlant_til || "—"}</td> <td class="mono small-text center">${g.utlansdato || "—"}</td> <td class="mono small-text center ${f ? "orange-text fw" : ""}">${g.innleveringsdato || "—"}${f ? " ⚠️" : ""}</td> <td class="center">${f ? '<span class="badge-forfalt">FORFALT</span>' : "OK"}</td> <td class="center"><button class="btn-action btn-edit" onclick="apneRediger('${g.id}')">✏️</button></td> </tr>`;
}).join(””);
}

// ── Bokser ─────────────────────────────────────────────────────────────────
async function lastInnBokser() {
const { data } = await db.from(“bokser”).select(”*”).order(“id”);
bokser = data || [];
if (document.getElementById(“bokser”)?.classList.contains(“active”)) visBokser();
// Update edit modal boks_id select if present
populerBoksSelect();
}

function populerBoksSelect() {
[“editBoksId”, “itemBoksId”].forEach(selId => {
...
const tbody = document.getElementById(“bokserBody”);
if (!tbody) return;
if (!bokser.length) {
tbody.innerHTML = ‘<tr><td colspan="5" class="empty-row">Ingen bokser — opprett en med ➕ Ny boks</td></tr>’;
return;
}
// Count gjenstander per boks
tbody.innerHTML = bokser.map(b => {
const gjenstander_i_boks = gjenstander.filter(g => g.boks_id === b.id);
const antall = gjenstander_i_boks.length;
const tilgj  = gjenstander_i_boks.reduce((s, g) => s + Math.max(0, (g.antall_totalt||1)-(g.antall_utlant||0)), 0);
return `<tr class="item-row"> <td class="tc mono"><a class="item-link" href="boks.html?id=${enc(b.id)}">${b.id}</a></td> <td class="tc"><a class="item-name-link" href="boks.html?id=${enc(b.id)}">${b.navn}</a></td> <td class="tc muted-text">${b.hylleplassering || "—"}</td> <td class="tc">${antall > 0 ? `<span class="green-text fw">${tilgj}</span>/<span class="muted-text">${antall} typer</span>` : '<span class="muted-text">Tom</span>'}</td> <td class="tc"> <div class="actions-cell"> <button class="btn-action btn-edit" onclick="window.location.href='boks.html?id=${enc(b.id)}'">📋 Åpne</button> <button class="btn-action btn-qr"   onclick="apneBoksQr('${b.id}','${b.navn.replace(/'/g,"\'")}')">⬛ QR</button> <button class="btn-action btn-delete" onclick="bekreftSlettBoks('${b.id}','${b.navn.replace(/'/g,"\'")}')">🗑️</button> </div> </td> </tr>`;
}).join(””);
}

function apneNyBoks() { document.getElementById(“nyBoksModal”).classList.add(“open”); }
function lukkNyBoks() { document.getElementById(“nyBoksModal”).classList.remove(“open”); }

async function lagreNyBoks() {
const id   = document.getElementById(“nyBoksId”).value.trim().toUpperCase();
const navn = document.getElementById(“nyBoksNavn”).value.trim();
const hylle = document.getElementById(“nyBoksHylle”).value.trim();
const beskr = document.getElementById(“nyBoksBeskrivelse”).value.trim();
if (!id || !navn) { visBanner(“ID og navn er påkrevd”, “error”); return; }
const btn = document.querySelector(”#nyBoksModal .btn-save”);
btn.disabled = true; btn.textContent = “Lagrer…”;
const { error } = await db.from(“bokser”).insert({ id, navn, hylleplassering: hylle || null, beskrivelse: beskr || null });
if (error) { visBanner(“Feil: “ + error.message, “error”); }
else {
lukkNyBoks();
await lastInnBokser();
visBanner(“✓ Boks opprettet! QR-kode kan genereres fra bokslisten.”, “success”);
settFane(“bokser”);
}
...
const cont = document.getElementById(“qrContainer”);
cont.innerHTML = “”;
const url = APP_BASE_URL + “boks.html?id=” + enc(id);
document.getElementById(“qrUrl”).textContent = url;
...
const { error } = await db.from(“bokser”).delete().eq(“id”, id);
if (!error) { lukkSlettModal(); await lastInnBokser(); visBanner(“🗑️ Boks slettet”, “success”); }
else visBanner(“Feil: “ + error.message, “error”);
};
}
...
// Populate boks select
const sel = document.getElementById(“itemBoksId”);
if (sel) {
sel.innerHTML = ‘<option value="">— Ingen boks —</option>’ +
bokser.map(b => `<option value="${b.id}">${b.navn} (${b.id})</option>`).join(””);
}
...
document.getElementById(“addBulkFields”).style.display = cb.checked ? “block” : “none”;
}
...
[“editModal”,“qrModal”,“deleteModal”,“scannerModal”,“leggTilModal”,“batchRedigerModal”,“nyBoksModal”].forEach(id => {
const el = document.getElementById(id);
if (!el) return;
