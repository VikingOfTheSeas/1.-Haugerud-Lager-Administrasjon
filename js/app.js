...
async function lastOppBilde(input) {
  ...
  // Vis lokal forhåndsvisning umiddelbart mens vi laster opp
  const lokalUrl = URL.createObjectURL(fil);
  oppdaterBildeForhandvis(lokalUrl);

  const ext     = fil.name.split('.').pop().toLowerCase();
  const filnavn = `hylle-${editingId || 'item'}-${Date.now()}.${ext}`;

  try {
    // Prøv Supabase Storage først
    const { data, error } = await db.storage
      .from("bilder")
      .upload(filnavn, fil, { upsert: true, contentType: fil.type });

    if (error) {
      // Supabase Storage ikke satt opp — les som base64 i stedet
      if (error.message.includes("bucket") || error.message.includes("not found") || error.statusCode === 404 || error.statusCode === 400) {
        await lagreSomBase64(fil, statusEl);
        return;
      }
      throw error;
    }
    ...
    statusEl.className = "bilde-upload-status success";
    statusEl.textContent = "✓ Bilde lastet opp til Supabase!";
...
async function lagreSomBase64(fil, statusEl) {
  // Komprimer og lagre som base64 data URL direkte i databasen
  statusEl.textContent = "⟳ Komprimerer bilde…";
  ...
  reader.onload = e => {
    img.onload = () => {
      // Maks 800px bred, behold proporsjoner
      const maxW = 800;
      const scale = Math.min(1, maxW / img.width);
      canvas.width  = Math.round(img.width  * scale);
      canvas.height = Math.round(img.height * scale);
      ...
      oppdaterBildeForhandvis(dataUrl);
      statusEl.className = "bilde-upload-status success";
      statusEl.textContent = "✓ Bilde klart! (Lagret lokalt — se guide for Supabase Storage)";
