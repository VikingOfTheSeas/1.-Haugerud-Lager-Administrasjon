...
function renderBoks() {
  ...
  if (!grupper.size) {
    ...
    tbody.innerHTML = innholdListe.map(gr => {
      ...
      return `<tr>
        <td style="padding-left:14px"><a class="item-name-link" href="item.html?id=${enc(gr.alle[0].id)}">${gr.navn}</a></td>
        <td class="tc green-text fw">${gr.tilgj}</td>
        <td class="tc ${gr.utlant > 0 ? "red-text" : "muted-text"}">${gr.utlant}</td>
        <td class="tc muted-text">${gr.alle.length}</td>
        <td class="tc" style="white-space:nowrap">
          <button class="btn-action btn-edit" style="padding:9px 13px;font-size:17px;min-width:42px" onclick="apneUtlanForNavn('${sn}')">📤</button>
          <button class="btn-action btn-qr"  style="padding:9px 13px;font-size:17px;min-width:42px" onclick="apneLeverForNavn('${sn}')">📥</button>
        </td>
      </tr>`;
    }).join("");
  }
  ...
  if (!utlånte.length) {
    ...
    uTbody.innerHTML = utListe.map(gr => {
      ...
      return `<tr class="${forfalt ? "row-forfalt" : ""}">
        ...
        <td class="tc mono small-text ${forfalt ? "orange-text fw" : ""}" style="white-space:nowrap">${gr.innleveringsdato}${forfalt ? " ⚠️" : ""}</td>
        <td class="tc" style="white-space:nowrap">
          ...
          <button class="btn-action btn-edit" style="padding:9px 13px;font-size:16px;min-width:42px" onclick="leverDels('${idStr}')" title="Lever inn delvis">✏️</button>
        </td>
      </tr>`;
    }).join("");
  }
...
function byggUtlanModal(forhåndsNavn) {
  ...
  const html = `
    <div class="form-field">
      ...
      <select id="utlanNavn" class="filter-input" style="width:100%" onchange="oppdaterMaks()">
        ${tilgjGrupper.map(([navn, gr]) =>
          `<option value="${navn}" data-maks="${gr.tilgj}"${navn === forhåndsNavn ? " selected" : ""}>${navn} (${gr.tilgj} tilgj.)</option>`
        ).join("")}
      </select>
    </div>
    <div class="form-field">
...
function apneLeverForNavn(navn)    { byggLeverModal(navn); }
function apneLeverForGjenstand(id) {
  const g = boksGjenstander.find(x => x.id === id);
  byggLeverModal(g ? g.navn : null);
}
...
function byggLeverModal(filtrertNavn) {
  ...
  for (const u of aktive) {
    ...
    html += `<div class="utlan-section" style="gap:8px">
      <div class="utlan-title">${navn} — ${u.utlant_til || "?"} (lånt ${u.antall} stk)</div>
...
function boksEtikettHTML(qrSrc) {
  return `<div class="print-label">
    <div class="print-label-qr"><img src="${qrSrc}" width="190" height="190" style="display:block"></div>
    <div class="print-label-info">
      <div class="print-label-navn">${currentBoks.navn}</div>
      <div class="print-label-id">${currentBoks.id}</div>
      ${currentBoks.hylleplassering ? `<div class="print-label-hylle">📍 ${currentBoks.hylleplassering}</div>` : ""}
