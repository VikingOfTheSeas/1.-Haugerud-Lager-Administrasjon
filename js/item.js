...
// ── Legg til i utvalg (for batch fra item-siden) ─────────────────────────
function toggleItemValg() {
  const btn  = document.getElementById("selectToggleBtn");
  const key  = "speider_valg";
  let valgt  = JSON.parse(sessionStorage.getItem(key) || "[]");
  const id   = currentItem?.id;
  if (!id) return;
  const idx  = valgt.indexOf(id);
  if (idx >= 0) {
    valgt.splice(idx, 1);
    btn.textContent = "☑ Legg til i utvalg";
    btn.classList.remove("qbtn-active");
  } else {
    valgt.push(id);
    btn.textContent = "✓ I utvalget (" + valgt.length + ")";
    btn.classList.add("qbtn-active");
  }
  sessionStorage.setItem(key, JSON.stringify(valgt));
}
