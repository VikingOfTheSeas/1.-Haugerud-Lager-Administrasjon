...
async function lagreBatchRediger() {
  ...
  for (const id of ids) {
    ...
    if (boksVal === "__fjern__") oppdatert.boks_id = null;
...
async function visBokser() {
  ...
  const sorterte = [...bokser].sort((a, b) => {
    const av = String(a[boksSortField] || "").toLowerCase();
    const bv = String(b[boksSortField] || "").toLowerCase();
    return boksSortDir === "asc" ? av.localeCompare(bv, "no") : bv.localeCompare(av, "no");
  });
  tbody.innerHTML = sorterte.map(b => {
    const gib    = gjenstander.filter(g => g.boks_id === b.id);
    const totalt = gib.length;
    const tilgj  = gib.filter(g => g.status === "Tilgjengelig").length;

    // Grupper per navn
    const navnMap = new Map();
    for (const g of gib) {
      if (!navnMap.has(g.navn)) navnMap.set(g.navn, { tilgj: 0, tot: 0 });
      const r = navnMap.get(g.navn);
      r.tot++;
      if (g.status === "Tilgjengelig") r.tilgj++;
    }
    const typer = navnMap.size;

    // Kompakt breakdown: "6× Kniv  2× Øks"
    const breakdown = [...navnMap.entries()]
      .map(([navn, r]) => {
        const all_tilgj = r.tilgj === r.tot;
        const none_tilgj = r.tilgj === 0;
        const clr = none_tilgj ? "color:var(--danger)" : (!all_tilgj ? "color:var(--warning)" : "color:#475569");
        return `<span style="${clr};white-space:nowrap">${r.tot}× ${navn}</span>`;
      })
      .join('<span style="color:#2d3748;padding:0 3px">·</span>');

    const gjenstanderCell = totalt === 0
      ? `<span class="muted-text small-text">Tom</span>`
      : `<div style="text-align:right">
           <span class="green-text fw">${tilgj}</span><span class="muted-text">/${totalt}</span>
           <span class="muted-text small-text" style="margin-left:4px">${typer} type${typer !== 1 ? "r" : ""}</span>
           <div style="font-size:10px;margin-top:3px;line-height:1.6;text-align:right">${breakdown}</div>
         </div>`;

    return `<tr class="item-row">
      <td class="tc mono"><a class="item-link" href="boks.html?id=${enc(b.id)}">${b.id}</a></td>
      <td><a class="item-name-link" href="boks.html?id=${enc(b.id)}">${b.navn}</a></td>
      <td class="muted-text">${b.hylleplassering || "—"}</td>
      <td style="padding-right:12px">${gjenstanderCell}</td>
      <td class="tc">
        <div class="actions-cell">
          <button class="btn-action btn-edit" onclick="window.location.href='boks.html?id=${enc(b.id)}'">📋 Åpne</button>
          <button class="btn-action btn-qr"   onclick="apneBoksQr('${b.id}','${b.navn.replace(/'/g,"\'")}')">⬛ QR</button>
          <button class="btn-action btn-delete" onclick="bekreftSlettBoks('${b.id}','${b.navn.replace(/'/g,"\'")}')">🗑️</button>
        </div>
      </td>
    </tr>`;
  }).join("");
...
let boksSortField = "id";
let boksSortDir   = "asc";

function setSorterBoks(felt) {
  boksSortDir   = boksSortField === felt && boksSortDir === "asc" ? "desc" : "asc";
  boksSortField = felt;
  visBokser();
}
