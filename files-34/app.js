...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Auto-open edit modal if redirected from item page
  const editId = new URLSearchParams(location.search).get("edit");
  if (editId) {
    history.replaceState({}, "", location.pathname);
    setTimeout(() => apneRediger(editId), 400);
  }
});
...
async function leggTil(e) {
  ...
  const hylle   = document.getElementById("itemHylle")?.value.trim() || "";
  ...
  if (error) { visBanner("Feil: " + error.message, "error"); }
    ...
    document.getElementById("itemHylle").value = "";
    oppdaterPrev();
    await lastInn();
    lukkLeggTilModal();
  }
...
async function visBokser() {
  ...
  tbody.innerHTML = bokser.map(b => {
    ...
    return `<tr class="item-row">
      ...
      <td class="tc">${antall > 0 ? `<span class="green-text fw">${tilgj}</span>/<span class="muted-text">${antall} typer</span>` : '<span class="muted-text">Tom</span>'}</td>
      <td class="tc">
        <div class="actions-cell">
...
function autofyllHylle() {
  const boksId = document.getElementById("itemBoksId")?.value;
  const boks = bokser.find(b => b.id === boksId);
  const hylleEl = document.getElementById("itemHylle");
  if (hylleEl && boks?.hylleplassering) {
    hylleEl.value = boks.hylleplassering;
  } else if (hylleEl && !boksId) {
    hylleEl.value = "";
  }
}
