...
function visTabell(liste) {
  ...
  tbody.innerHTML = liste.map(g => {
    ...
    return `<tr class="item-row row-${sk(g.status)}" data-id="${g.id}">
      <td><input type="checkbox" class="row-checkbox" value="${g.id}" onchange="oppdaterValg()"></td>
      ...
      <td>
        <div class="actions-cell">
          ...
          <button class="btn-action btn-select" onclick="toggleValgRow('${g.id}',this)" title="Velg">☑</button>
        </div>
      </td>
    </tr>`;
  }).join("");
...
async function leggTil(e) {
  ...
  const nye = Array.from({ length: antall }, (_, i) => ({
    ...
    hylleplassering: "", notater: not, bilde_url: bildeUrl,
  }));
...
function oppdaterValg() {
  ...
  document.querySelectorAll(".row-checkbox:checked").forEach(cb => {
    if (cb.id !== "selectAll") valgte.add(cb.value);
  });
...
function toggleValgRow(id, btn) {
  const cb = document.querySelector(`.row-checkbox[value="${id}"]`);
  if (cb) { cb.checked = !cb.checked; oppdaterValg(); }
}

function toggleSelectAll(el) {
  document.querySelectorAll(".row-checkbox").forEach(cb => {
    if (cb.id !== "selectAll") cb.checked = el.checked;
  });
  oppdaterValg();
}
...
async function batchUtlan() {
  if (!valgte.size) return;
  // Åpne rediger-modal for første valgte, og la brukeren fylle inn utlånsinfo
  // som da kopieres til alle valgte
  const ids = [...valgte];
  const g = gjenstander.find(x => x.id === ids[0]);
  if (!g) return;
  // Sett flagg om at dette er batch-operasjon
...
// Patch lagreRediger to handle batch
const _origLagreRediger = lagreRediger;
lagreRediger = async function() {
